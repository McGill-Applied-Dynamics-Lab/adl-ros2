        ...
        if mProgramState.isRunning:
            ...
            if cur_packet.info is not None:
                ...
                if mTaskState.baseline == True:
                    ...
                    mPacket = local_processes.RIMPacket(baselinePacket, hl, cur_packet.delay, mSimulationConfig.latencyModel, 0)
                    ...
                    phi_velocity = np.array(baselinePacket.info.drone_com_velocity[0:3]) - inverse3_state.inverse3_velocity
                    raw_force = - mPacket.reduced_model.stiffness * phi_position - mPacket.reduced_model.damping * phi_velocity
                    raw_force.shape = (3, 1)
                ...
                if abs(rendered_force[0,0]) > mCap or abs(rendered_force[1,0]) > mCap or abs(rendered_force[2,0]) > mCap:
