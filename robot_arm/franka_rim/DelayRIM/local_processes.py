...
class RIMPacket:
    def __init__(self, packet, hl, delay, RIMType, key):
        ...
        Id3 = np.identity(3)
        augmented_mass_matrix = (self.params.effective_mass + hl * (
                self.reduced_model.damping + hl * self.reduced_model.stiffness) * Id3)
        inverse_augmented_mass_matrix = np.linalg.inv(augmented_mass_matrix)
        mass_factor = math_helper_functions.multiply_matrix(inverse_augmented_mass_matrix,
...
def oneStepUpdate(mPacket, inverse3_position, inverse3_velocity):
...
def getForce(mPacket):
    ...
    mPacket.reduced_model.interface_force = -mPacket.reduced_model.stiffness * mPacket.reduced_model.phi_position - (
                mPacket.hl * mPacket.reduced_model.stiffness + mPacket.reduced_model.damping) * mPacket.reduced_model.phi_velocity
    mPacket.reduced_model.interface_force.shape = (3, 1)

def getZOHForce(mPacket):
    ...
    mPacket.reduced_model.phi_position = mPacket.reduced_model.reduced_model_position - mPacket.inverse3_position
    mPacket.reduced_model.phi_velocity = mPacket.reduced_model.reduced_model_velocity - mPacket.inverse3_velocity
    mPacket.reduced_model.interface_force = - mPacket.reduced_model.stiffness * mPacket.reduced_model.phi_position - mPacket.reduced_model.damping * mPacket.reduced_model.phi_velocity
    mPacket.reduced_model.interface_force.shape = (3, 1)
...
def oneStep(mPacket):
    ...
    regular_force_terms = mPacket.params.effective_mass @ (mPacket.inverse3_acceleration) / mPacket.hl + mPacket.params.effective_force - mPacket.reduced_model.stiffness * mPacket.reduced_model.phi_position + (
                                  mPacket.reduced_model.damping + mPacket.hl * mPacket.reduced_model.stiffness) * mPacket.inverse3_velocity
    mPacket.reduced_model.reduced_model_velocity = mPacket.mass_factor @ mPacket.reduced_model.reduced_model_velocity + mPacket.hl * mPacket.inverse_augmented_mass_matrix @ regular_force_terms
    mPacket.reduced_model.reduced_model_position = mPacket.reduced_model.reduced_model_position + mPacket.hl * mPacket.reduced_model.reduced_model_velocity
...
def makeCatchUpProcess(key):
    ...
    p = Process(target=catchUpProcess, args=(child_conn, key,))
    p.start()
...
class delayRIMModel:
    ...
    def startCatchUp(self, packet, inverse3_position, inverse3_velocity, hl, delay, RIMType):
        if (len(self.inactiveProcesses) > 0):
            ...
            self.allConnections[process].send([CatchUpCommand.CATCHUP, inverse3_position, inverse3_velocity, packet, hl, delay, RIMType])
            ...
            self.allConnections[self.processKeys[-1]].send([CatchUpCommand.CATCHUP, inverse3_position, inverse3_velocity, packet, hl, delay, RIMType])

    def getUpdate(self, inverse3_position, inverse3_velocity):
    ...
    def getForceFromData(self, packet, hl, delay, RIMType, recent_position, recent_velocity):
        #print('starting catch up')
        catchUpData = self.getUpdate(recent_position, recent_velocity)
