...
#include "fr3_controllers/my_controller.hpp"
...
controller_interface::CallbackReturn RobotController::on_init()
{
    ...
    RCLCPP_INFO(
        get_node()->get_logger(),
        "command_interface_types_: [%s]", 
        std::accumulate(
            command_interface_types_.begin(), command_interface_types_.end(), std::string(),
            [](const std::string &a, const std::string &b) -> std::string {
                return a.empty() ? b : a + ", " + b;
            }
        ).c_str()
    ...
    RCLCPP_INFO(
        get_node()->get_logger(),
        "state_interface_types_: [%s]", 
        std::accumulate(
            state_interface_types_.begin(), state_interface_types_.end(), std::string(),
            [](const std::string &a, const std::string &b) -> std::string {
                return a.empty() ? b : a + ", " + b;
            }
        ).c_str()
  ...
  const rclcpp::Time & time, const rclcpp::Duration & /*period*/)
{
  ...
  if (trajectory_msg_ != nullptr)
  {
    ...
    // for (size_t i = 0; i < point_interp_.positions.size(); ++i)
    // {
    //   RCLCPP_INFO(get_node()->get_logger(), "point_interp_.positions[%zu] = %f", i, point_interp_.positions[i]);
    // }

    RCLCPP_INFO(get_node()->get_logger(), "Current trajectory time: %f seconds", (time - start_time_).seconds());

    // for (size_t i = 0; i < joint_position_command_interface_.size(); i++)
    // {
    //   joint_position_command_interface_[i].get().set_value(point_interp_.positions[i]);
    // }
    joint_position_command_interface_[3].get().set_value(point_interp_.positions[3]);
...
PLUGINLIB_EXPORT_CLASS(
  fr3_controllers::RobotController, controller_interface::ControllerInterface)
