...
#include <cmath>
#include <exception>
#include <fr3_controllers/cartesian_vel_controller.hpp>
...
CallbackReturn CartesianVelController::on_configure(const rclcpp_lifecycle::State& /*previous_state*/)
{
...
controller_interface::InterfaceConfiguration CartesianVelController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration CartesianVelController::state_interface_configuration() const
{
  controller_interface::InterfaceConfiguration config;
  config.type = controller_interface::interface_configuration_type::INDIVIDUAL;
...
CallbackReturn CartesianVelController::on_activate(const rclcpp_lifecycle::State& /*previous_state*/)
{
  ...
  // std::tie(start_orientation_, start_position_) = franka_cartesian_velocity_->getCurrentOrientationAndTranslation();
...
controller_interface::CallbackReturn CartesianVelController::on_deactivate(
...
controller_interface::return_type CartesianVelController::update(const rclcpp::Time& /*time*/,
...
void CartesianVelController::initialize_controller()
{
  // Get initial orientation and translation
  // std::tie(start_orientation_, x_ee_start_) = franka_cartesian_velocity_->getCurrentOrientationAndTranslation();
  ...
  CmdType start_twist;
  start_twist.twist.linear.x = 0.0;
  start_twist.twist.linear.y = 0.0;
  start_twist.twist.linear.z = 0.0;
  start_twist.twist.angular.x = 0.0;
  start_twist.twist.angular.y = 0.0;
  start_twist.twist.angular.z = 0.0;

  received_command_msg_ptr_.set(std::make_shared<CmdType>(start_twist));
...
void CartesianVelController::update_desired_command()
{
  if (!new_cmd_msg_)
  {
    return;
  ...
  dx_ee_des_(0) = last_command_msg->twist.linear.x;
...
void CartesianVelController::update_current_state(double dt)
{
  ...
  // std::tie(current_orientation, current_position) =
  // franka_cartesian_velocity_->getCurrentOrientationAndTranslation(); x_ee_(0) = current_position(0); x_ee_(1) =
  // current_position(1); x_ee_(2) = current_position(2);
...
void CartesianVelController::print_robot_states()
{
  ...
  RCLCPP_INFO(get_node()->get_logger(), "\tdx_ee_des:  \t(%10.6f, %10.6f, %10.6f)", dx_ee_des_(0), dx_ee_des_(1),
              dx_ee_des_(2));
...
Vector3d CartesianVelController::clamp(Vector3d value, Vector3d min_val, Vector3d max_val)
{
...
// NOLINTNEXTLINE
PLUGINLIB_EXPORT_CLASS(fr3_controllers::CartesianVelController, controller_interface::ControllerInterface)
