...
def main():
    ...
    # 1) Move to a safe start position.
    # Since joint controller is active, move_to() will route through IK + joint trajectory execution.
    # start_position = np.array([0.45, 0.0, 0.45])
    # robot.move_to(position=start_position, time_to_move=3.0)

    start_pose = robot.end_effector_pose.copy()

    # 2) Build Cartesian waypoint sets (outside Robot class).
    ...
    waypoints_2 = generate_linear_waypoints(
        ...
        end_position=end_pose_1.position + np.array([0.1, 0.0, 0.0]),
