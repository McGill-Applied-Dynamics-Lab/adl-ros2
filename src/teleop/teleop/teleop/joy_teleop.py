...
from teleop.Gamepad import XBOX_GAMEPAD_MAP
...
# TODO: from robot_arm_interface.utils import motion2rostwist, rostwist2motion, rospose2se3, se32rospose
...
class JoyTeleop(Node):
    def __init__(self):
        ...
        self._in_angular_mode = False  # True if LT is pressed

        # Robot
        self._control_mode = ControlModes.POSITION

        self.X_G = pin.SE3.Identity()  # Gripper pose in the world frame

        self._ee_center = np.zeros(3)  # Center position for the ee
        self._ee_des = np.zeros(3)  # Desired end effector position

        self._ee_rpy_center = np.zeros(3)  # Center rpy for the ee
        self._ee_rpy_des = np.zeros(3)  # Desired end effector rpy

        self._position_scale = 0.1  # Scale for the position of the joystick to the robot
        self._angle_scale = 0.5  # Scale for the angle of the joystick to the robot
        self._lin_vel_scale = 0.1  # Scale for the velocity of the joystick to the robot
        self._ang_vel_scale = 0.5  # Scale for the velocity of the joystick to the robot

        # Gripper
        self._button_debounce_time = 1.0
        self._last_press_B = 0
        self._gripper_toggle_ac = ActionClient(self, GripperToggle, "gripper_toggle")
        self.get_logger().info("Waiting for action servers...")
        self._gripper_toggle_ac.wait_for_server(timeout_sec=2.0)
        ...
        self.get_logger().info("JoyTeleop initialized.")
        self.get_logger().info(
            "Control the robot position with the joystick. Press RT to control the velocity. Press LT to control the orientation."
        )
    ...
    def _calibrate(self):
        ...
        self._ee_rpy_center /= self._n_robot_cal_msgs
        self.get_logger().info(f"Robot position center: {self._ee_center}")
        self.get_logger().info(f"Robot rpy center: {self._ee_rpy_center}")
    ...
    def _joy_topic_callback(self, msg: Joy):
        ...
        if self._buttons_values[XBOX_GAMEPAD_MAP.buttons.B.value] == 1:
            self.get_logger().info("Button B pressed!")

            # Check debounce
            if (current_time - self._last_press_B) > self._button_debounce_time:
                self.get_logger().info("Toggling gripper from B button press")
                self._last_press_B = current_time
                self.gripper_toggle()
    ...
    def _robot_topic_callback(self, msg: PoseStamped):
        ...
        if self._calibrating:
            self._ee_center += self.X_G.translation
            self._ee_rpy_center += pin.rpy.matrixToRpy(self.X_G.rotation)
    ...
    def _compute_ee_command(self):
        ...
        self._in_angular_mode = self._axes_values[JoyAxisMap.LT.value] == -1.0

        #! Linear command
        if not self._in_angular_mode:
            if self._control_mode == ControlModes.POSITION:
                ee_des = self._compute_ee_des()
                ee_rpy_des = self._ee_rpy_center

                self._ee_cmd_msg.control_mode = ControlModes.POSITION.value

                self._ee_cmd_msg.ee_des = Pose()  # To init to 0
                ...
                R = pin.rpy.rpyToMatrix(ee_rpy_des)
                q = pin.Quaternion(R).coeffs()
                self._ee_cmd_msg.ee_des.orientation.x = q[0]
                self._ee_cmd_msg.ee_des.orientation.y = q[1]
                self._ee_cmd_msg.ee_des.orientation.z = q[2]
                self._ee_cmd_msg.ee_des.orientation.w = q[3]

                self._ee_cmd_msg.ee_vel_des = Twist()  # Set vel to 0
        ...
        #! Angular command
            ...
            if self._control_mode == ControlModes.POSITION:
                ee_ang = self._compute_ee_ang()

                self._ee_cmd_msg.control_mode = ControlModes.POSITION.value

                R = pin.rpy.rpyToMatrix(ee_ang)
                t = self._ee_center
                X_G_des = pin.SE3(R, t)
                q = pin.Quaternion(X_G_des.rotation).coeffs()

                self._ee_cmd_msg.ee_des.position.x = t[0]
                self._ee_cmd_msg.ee_des.position.y = t[1]
                self._ee_cmd_msg.ee_des.position.z = t[2]
                ...
                ee_ang_vel_des = self._compute_ee_ang_vel()

                self._ee_cmd_msg.control_mode = ControlModes.VELOCITY.value

                self._ee_cmd_msg.ee_vel_des = Twist()

                self._ee_cmd_msg.ee_vel_des.angular.x = ee_ang_vel_des[0]
                self._ee_cmd_msg.ee_vel_des.angular.y = ee_ang_vel_des[1]
                self._ee_cmd_msg.ee_vel_des.angular.z = ee_ang_vel_des[2]
    ...
    def _compute_ee_vel(self) -> np.ndarray:
        ...
        # Compute the desired end effector position
        ee_vel_des = self._lin_vel_scale * js_axes

        return ee_vel_des.astype(float)

    def _compute_ee_ang(self):
        # Get the interesting axes
        js_axes = self._axes_values[[JoyAxisMap.LS_V.value, JoyAxisMap.LS_H.value, JoyAxisMap.RS_H.value]]

        # Apply the deadzone
        js_axes = np.where(np.abs(js_axes) < self._js_deadzone_thres, 0, js_axes)
        js_axes = np.array([-js_axes[0], -js_axes[1], -js_axes[2]])

        # Compute the desired end effector position
        ee_ang = self._ee_rpy_center + self._angle_scale * js_axes

        return ee_ang  # rpy angles

    def _compute_ee_ang_vel(self) -> np.ndarray:
        # Get the interesting axes
        js_axes = self._axes_values[[JoyAxisMap.LS_V.value, JoyAxisMap.LS_H.value, JoyAxisMap.RS_H.value]]
        ...
        # Compute the desired end effector position
        ee_ang_vel_des = self._ang_vel_scale * js_axes

        return ee_ang_vel_des.astype(float)
