...
class AcousticRecorderNode(Node):
    ...
    MODIFIED: supports saving BOTH RF channels per probe segment.
      - start_segment(t0_perf) starts a segment window
      - stop_segment() returns:
          {
            "t0_perf": ...,
            "bbb0_ms": {"1":..., "2":...} (if available),
            "packets_by_rf": { "1":[...], "2":[...] }
          }

    Receiver detection:
      - wait_for_first_packet(timeout): any packet
      - wait_for_both_rf(timeout): must see rf_id 1 and 2 at least once
      - last_rx_age_sec()
    """
    def __init__(self, topic="/acoustic/raw", max_packets=200000, context=None, store_raw=True):
        ...
        # Match publisher QoS (sensor data is typically BEST_EFFORT)
        ...
        self._rx_evt_any = threading.Event()
        self._seen_rf = {1: False, 2: False}
        self._rx_evt_both = threading.Event()
    ...
    def _cb(self, msg: AcousticPacket):
        ...
        with self._lock:
            ...
            self._rx_evt_any.set()

            if rf in self._seen_rf:
                self._seen_rf[rf] = True
                if self._seen_rf[1] and self._seen_rf[2]:
                    self._rx_evt_both.set()

    def wait_for_first_packet(self, timeout_sec: float = 5.0) -> bool:
        with self._lock:
            if self._rx_count > 0:
                return True
        return self._rx_evt_any.wait(timeout=timeout_sec)

    def wait_for_both_rf(self, timeout_sec: float = 5.0) -> bool:
        with self._lock:
            if self._seen_rf[1] and self._seen_rf[2]:
                return True
        return self._rx_evt_both.wait(timeout=timeout_sec)
    ...
    def stop_segment(self):
        with self._lock:
            if not self._seg_active:
                return {"t0_perf": None, "bbb0_ms": {"1": None, "2": None}, "packets_by_rf": {"1": [], "2": []}}
            ...
            # reference BBB timestamp per rf
            bbb0 = {
                "1": int(by_rf[1][0]["time_offset_ms"]) if len(by_rf[1]) else None,
                "2": int(by_rf[2][0]["time_offset_ms"]) if len(by_rf[2]) else None,
            }
            ...
            for rf in (1, 2):
                b0 = bbb0[str(rf)]
                for p in by_rf[rf]:
                    p["t_rel_perf"] = float(p["t_abs_perf"] - t0)
                    if b0 is not None:
                        p["t_rel_bbb"] = float((p["time_offset_ms"] - b0) / 1000.0)
                    else:
                        p["t_rel_bbb"] = None
        ...
        # return with string keys for pickle friendliness / readability
        return {
            "t0_perf": t0,
            "bbb0_ms": bbb0,
            "packets_by_rf": {"1": by_rf[1], "2": by_rf[2]},
        }
...
def main():
    ...
    # Fail fast if nothing is arriving at all
    ...
    # Optional but recommended: ensure BOTH rf_id=1 and rf_id=2 are present before probing
    if not ac_node.wait_for_both_rf(timeout_sec=5.0):
        stop_evt.set()
        spin_th.join(timeout=1.0)
        try:
            ac_node.destroy_node()
        ...
        try:
            ...
            pass
        raise RuntimeError(
            "Acoustic stream detected, but did not see BOTH rf_id=1 and rf_id=2 within 5s.\n"
            "Check your BBB loop is sending rf_id=1 then rf_id=2 frames."
        )
    ...
    try:
        ...
        exp_dict = {
            ...
            "landmarks": landmarks,
            "z_offset": Z_OFFSET,
            "acoustic": [],  # now stores BOTH rf channels per probe
        }
        ...
        for i, loc in enumerate(grid_xy_world):
            ...
            pkts1 = acoustic_segment.get("packets_by_rf", {}).get("1", [])
            pkts2 = acoustic_segment.get("packets_by_rf", {}).get("2", [])

            # Require BOTH channels to have data during this probe
            if len(pkts1) == 0 or len(pkts2) == 0:
                raise RuntimeError(
                    f"Probe {i + 1}: missing RF channel data during probe window.\n"
                    f"  RF1 packets: {len(pkts1)}\n"
                    f"  RF2 packets: {len(pkts2)}\n"
                    f"Last RX age = {ac_node.last_rx_age_sec():.3f}s\n"
                    "Check: TCP connection, BBB streaming loop, rf_id framing, /acoustic/raw publisher health."
                )
