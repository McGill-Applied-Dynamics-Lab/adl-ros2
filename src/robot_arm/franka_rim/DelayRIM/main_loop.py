...
if "Ranking" not in multi_user_data.columns:
    multi_user_data["Ranking"] = [0] * len(multi_user_data.index)
for maneuver in range(0, num_maneuvers):
    for task in range(0, num_tasks):
        task_row = multi_user_data.loc[
            (multi_user_data["User"] == mTaskState.user)
            & (multi_user_data["Maneuver"] == maneuver)
            & (multi_user_data["Task"] == task)
        ].index
        if math.isnan(multi_user_data.at[task_row.values[0], "Ranking"]):
            multi_user_data.at[task_row.values[0], "Ranking"] = 0
...
def maneuver_dict_u(num):
    dict = {0: "Free_Movement", 1: "Guided_Movement"}
    return dict[num]
...
def get_rank_text():
    [com, latency, delay] = get_config(multi_user_data, mTaskState.user, mTaskState.maneuver, mTaskState.task)
    output_text = latency
    output_text = "Task"
    for maneuver in range(0, num_maneuvers):
    ...
    for task in range(0, num_tasks):
        ...
        for maneuver in range(0, num_maneuvers):
            output_text = (
                output_text + f" {mTaskState.rank[maneuver][task]}/10    " + " " * len(maneuver_dict(maneuver))
            )
        output_text = output_text + "\n"
    return output_text
...
def update_graphics(
    ...
    rod1.axis = vp.vector(-2.0 * rod1_axis[0], -2.0 * rod1_axis[1], -2.0 * rod1_axis[2])
    rod1.pos = vp.vector(rod1_com[0] + rod1_axis[0], rod1_com[1] + rod1_axis[1], rod1_com[2] + rod1_axis[2])

    rod2.axis = vp.vector(-2.0 * rod2_axis[0], -2.0 * rod2_axis[1], -2.0 * rod2_axis[2])
...
def display_setup(drone_radius, rod_length, rod_radius, load_radius, multi_user_data):
    ...
    scene1.height = 600
    scene1.range = 1.8
    scene1.forward = vp.vector(0.0, 1.0, 0.0)
    scene1.center = vp.vector(0.0, 0.0, 0.0)
    scene1.up = vp.vector(0.0, 0.0, 1.0)
    scene1.camera.pos = vp.vector(0.0, -0.7, -0.1)
    ...
    drone = vp.sphere(
        ...
        axis=vp.vector(0.0, 0.0, -1.0),
        radius=drone_radius,
        color=vp.color.blue,
        opacity=0.5,
    )
    drone_com = np.array([0.0, 0.0, 0.0])
    drone_com.shape = (3, 1)

    end_effector = vp.sphere(
        pos=vp.vector(0.0, 0.0, 0.0),
    ...
    end_effector_label = vp.label(
        pos=end_effector.pos,
        text=f"{end_effector.pos.x:.2f},{end_effector.pos.y:.2f},{end_effector.pos.z:.2f}",
        xoffset=-10,
        yoffset=20,
        opacity=0,
        height=10,
    ...
    rod1 = vp.cylinder(
        pos=vp.vector(0, 0.0, -drone_radius),
        axis=vp.vector(0.0, 0.0, -1.0),
        length=rod_length,
        radius=rod_radius,
        color=vp.color.white,
    )
    rod1_com = np.array([0.0, 0.0, -drone_radius - rod_length / 2.0])
    rod1_com.shape = (3, 1)

    rod2 = vp.cylinder(
        pos=vp.vector(0.0, 0.0, -drone_radius - rod_length),
        axis=vp.vector(0.0, 0.0, -1.0),
        length=rod_length,
        radius=rod_radius,
        color=vp.color.white,
    )
    rod2_com = np.array([0.0, 0.0, -drone_radius - 3.0 * rod_length / 2.0])
    rod2_com.shape = (3, 1)

    load = vp.sphere(
        pos=vp.vector(0.0, 0.0, -drone_radius - 2 * rod_length - load_radius),
        axis=vp.vector(0.0, 0.0, -1.0),
        radius=load_radius,
        color=vp.color.red,
    )
    load_com = np.array([0.0, 0.0, -drone_radius - 2.0 * rod_length - load_radius])
    load_com.shape = (3, 1)
    ...
    scene1.append_to_title("<b>Multi-User Study</b>\n")
    vp.button(
        canvas=scene1,
        bind=B,
        text="\n  Start  \n\n",
    ...
    vp.checkbox(canvase=scene1, bind=MBaseline, pos=vp.scene.title_anchor, checked=True)
    mTaskState.baseline = True
...
class VirtualPacket:
    def __init__(
        self, effective_params=virtual_environment.EffectiveParams(), virtual_state=virtual_environment.VirtualState()
    ):
        self.effective_params = effective_params
        self.virtual_state = virtual_state


def VirtualEnvironmentProcess(
    connection, hv, mass_drone, drone_radius, mass_density, rod_length, rod_radius, mass_load, load_radius
):
    # Create virtual environment
    mVirtualEnvironment = virtual_environment.VirtualEnvironment(
        hv=hv,
    ...
    while True:
        ...
        virtual_packet = VirtualPacket(effective_params, virtual_state)
        connection.send(virtual_packet)
        inverse_state: virtual_environment.Inverse3State = connection.recv()  # waits until the information is received
...
def send_force_to_Inverse3(Inverse3, rendered_force, inverse3_initial_position):
    ...
    inverse3_position = np.asarray(
        [
            inverse3_raw_position[0] - inverse3_initial_position[0],
...
def saveData(trajectory, forces):
    ...
    filename = f"UserOutput/User_{mTaskState.user}/Maneuver_{maneuver_dict_u(mTaskState.maneuver)}_Task_{latency}_{stable}_{current_time}.csv"
    # Create header row
    headerRow = ["X", "Y", "Z", "Fx", "Fy", "Fz"]

    if os.path.exists(f"UserOutput/User_{mTaskState.user}") == False:
        os.mkdir(f"UserOutput/User_{mTaskState.user}")
...
def main():
    ...
    drone, rod1, rod2, load, end_effector, end_effector_label, scene1, taskball, saveButton, ranks = display_setup(
        drone_radius, rod_length, rod_radius, load_radius, multi_user_data
    )
    sleep(2)
    ...
    while True:
        ...
        if mProgramState.isStarting:
            communicationModel, latencyModel, constantDelay = get_config(
                multi_user_data, mTaskState.user, mTaskState.maneuver, mTaskState.task
            )
            ...
            virtualProcess = Process(
                target=VirtualEnvironmentProcess,
                args=(
                    connectionVirtual,
                    hl * multirate_factor,
                    mass_drone,
            ...
            sleep(2)
            mModel = local_processes.delayRIMModel(8)
            sleep(2)
        ...
        if mProgramState.isRunning:
            # Every rate timesteps, get info from VirtualEnv and send it the I3 state
            if count % multirate_factor == 0:
                # receive state from connection
                virtual_packet: VirtualPacket = connectionLocal.recv()  # State of virtual env
            ...
            # Send the effective parameters to the communication line (simulate the delays)
            ...
            update_graphics(
                ...
                rod2,
                load,
                inverse3_state.inverse3_position,
                virtual_state.rod1_com,
                virtual_state.rod2_com,
                virtual_state.drone_com,
                virtual_state.load_com,
                virtual_state.drone_axis,
                virtual_state.rod1_axis,
                virtual_state.rod2_axis,
            ...
            if mSimulationConfig.performTask:
                ...
                if count * hl > 2 and savedFlag == False:
                    ...
                    scene1.background = vp.color.gray(0.2)
                if count * hl < 4:
                    taskball.pos = vp.vector(
                        0.0,
                        0.0,
                        0,
                    )
                elif count * hl < 6:
                    taskball.pos = vp.vector(0.0, 0.0, 0.25 * math.sin((count * hl - 6) * math.pi))
                elif count * hl < 8:
                    taskball.pos = vp.vector(0.0, 0.0, 0.0)
                elif count * hl < 10:
                    taskball.pos = vp.vector(0.0, 0.25 * math.sin((count * hl - 8) * math.pi), 0.0)
                elif count * hl < 12:
                    taskball.pos = vp.vector(0.0, 0.0, 0.0)
                elif count * hl < 14:
                    taskball.pos = vp.vector(0.25 * math.sin((count * hl - 10) * math.pi), 0.0, 0.0)
                elif count * hl <= 16:
                    taskball.pos = vp.vector(0.0, 0.0, 0.0)
            ...
            #! Record the state and force for saving
            # Between 2 and 16 seconds, record i3 state and rendered force every 10 steps
            ...
            cur_packet: Packet = communicationLine.getCurrent()
            ...
            if cur_packet.info is not None:
                ...
                #! Get Rendered Force from the DelayRIM
                rendered_force = mModel.getForceFromData(
                    cur_packet,
                    hl,
                    cur_packet.delay,
                    mSimulationConfig.latencyModel,
                    inverse3_state.inverse3_position,
                    inverse3_state.inverse3_velocity,
                )
                # print(f'obtained force time: {time.perf_counter()-tic}')

                # If we're in baseline mode, we use the last packet from the communication line
                if mTaskState.baseline:
                    baselinePacket = communicationLine.seeLast()
                    # (self, packet, hl, delay, RIMType, key):
                    mPacket = local_processes.RIMPacket(
                        baselinePacket, hl, cur_packet.delay, mSimulationConfig.latencyModel, 0
                    )
                    mPacket.reduced_model.reduced_model_position.shape = (3, 1)
                    mPacket.reduced_model.reduced_model_velocity.shape = (3, 1)

                    phi_position = np.array(baselinePacket.info.drone_com) - inverse3_state.inverse3_position
                    phi_velocity = (
                        np.array(baselinePacket.info.drone_com_velocity[0:3]) - inverse3_state.inverse3_velocity
                    )
                    raw_force = (
                        -mPacket.reduced_model.stiffness * phi_position - mPacket.reduced_model.damping * phi_velocity
                    )
                    raw_force.shape = (3, 1)
                ...
                if (
                    abs(rendered_force[0, 0]) > mCap
                    or abs(rendered_force[1, 0]) > mCap
                    or abs(rendered_force[2, 0]) > mCap
                ):
                    mProgramState.isSafe = False

                #! Apply rendered force to I3
