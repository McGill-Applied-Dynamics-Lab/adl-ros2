...
class SerialWorker(QThread):
    # Signal to send (chunk1, chunk2, chunk3, chunk_rate_hz, dt_since_last_us) to the GUI thread.
    # See teensy_tx.ino for the wire format of each 'C' chunk:
    #   uint32 chunk_size + chunk_size*uint16 ch1 + chunk_size*uint16 ch2
    #     + chunk_size*uint16 ch3 + float32 chunk_rate_hz + uint32 dt_since_last_us
    data_ready = pyqtSignal(np.ndarray, np.ndarray, np.ndarray, float, int)
                ...
                if self.ser.in_waiting > 0:
                    ...
                    if marker == b"B":
                        ...
                        if (
                            ...
                            ch3_data = np.frombuffer(ch3_bytes, dtype=np.uint16)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Teensy 10kHz Three-Channel Plotter")
    ...
    def init_ui(self):
        ...
        self.curve1 = self.plot_widget.plot(
            self.y1_data, pen=pg.mkPen("cyan", width=1.5), name="Channel 1 (pin 14)"
        )
        self.curve2 = self.plot_widget.plot(
            self.y2_data, pen=pg.mkPen("magenta", width=1.5), name="Channel 2 (pin 15)"
        )
        self.curve3 = self.plot_widget.plot(
            self.y3_data, pen=pg.mkPen("yellow", width=1.5), name="Channel 3 (pin 16)"
        )
    ...
    @pyqtSlot(np.ndarray, np.ndarray, np.ndarray, float, int)
    def update_plot(self, new_ch1, new_ch2, new_ch3, chunk_rate_hz, dt_since_last_us):
        ...
        self.y3_data = np.roll(self.y3_data, -chunk_len)
        ...
        self.curve3.setData(self.y3_data)
